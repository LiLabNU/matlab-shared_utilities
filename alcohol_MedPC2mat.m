function     [data, trialTS, AnimalIDcell] = alcohol_MedPC2mat(MedPCfile, folderDir, dur)
...
for fileNumber = 1:length(MedPCfile)
    ...
    for k = 1:length(keys)
        ...
        if col_idx == alcoholOn
            for numentry = 1:length_temp
                startIdx = round(data_matrix(numentry, alcoholOn)*100);
                endIdx = round(data_matrix(numentry, alcoholOff)*100);
                endIdx = min(endIdx, dur);
                data.(output_matrix_name)(fileNumber, startIdx:endIdx) = 1;
            end
        elseif col_idx == waterOn
            for numentry = 1:length_temp
                startIdx = round(data_matrix(numentry, waterOn)*100);
                endIdx = round(data_matrix(numentry, waterOff)*100);
                endIdx = min(endIdx, dur);
                data.(output_matrix_name)(fileNumber, startIdx:endIdx) = 1;
            end
%         else
