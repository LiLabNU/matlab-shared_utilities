function     [data, trialTS, AnimalIDcell] = SucroseSA_MedPC2mat(MedPCfile, folderDir, dur)
...
% \  I = Sucrose triggering nose poke Timestamp
% \  N = Port exit time stamp array
% \  L = array for port-entry timestamps
% \  N = array for port-exit timestamps
% \  P = array for cue onset timestamps

%%% for binary data extraction
PortEntry = 1;
PortExit = 2;
% Initialize the data_map with direct mappings
data_map = containers.Map({'L:', 'N:','F:', 'E:', 'H:', 'I:'}, ...
    [PortEntry, PortExit, 3, 4, 5, 6]);
% Initialize data_map2 with mappings for the structure fields
data_map2 = containers.Map([PortEntry, 3, 4, 5, 6], ...%  
    {'PortEntry', 'Reward', 'ActiveNP', 'InactiveNP', 'RewardTrigNP'});

%%% for timestamps extraction
% Initialize the timestamp_map with direct mappings
timestamp_map = containers.Map({'L:', 'N:','F:', 'E:', 'H:', 'I:' 'P:'}, ...
    [1,2,3,4,5,6,7]);
% Initialize timestamp_map2 with mappings for the structure fields
timestamp_map2 = containers.Map([1,2,3,4,5,6,7], ... %
    {'PortEntry','PortExit', 'Reward', 'ActiveNP', 'InactiveNP', 'RewardTrigNP', 'ToneOnset'});
